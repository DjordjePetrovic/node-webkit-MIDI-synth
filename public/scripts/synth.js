...
Synth = function (audioContext){
  ...
  window.audioContext = audioContext || new webkitAudioContext();
  ...
  this.filter = window.audioContext.createBiquadFilter();
  ...
  this.lfo = window.audioContext.createOscillator();
  this.lfo.noteOn(0); 

  this.oscLFOGain = window.audioContext.createGain();
  this.oscLFOGain.gain.value = 1.0;
  this.lfo.connect(this.oscLFOGain);
  this.oscLFOGain.connect(this.oscillator.detune);
  this.gain.connect(window.audioContext.destination);
...
Synth.prototype.noteOn = function(note){
  ...
  var frequency = this.notes[note] || 0,
      now = window.audioContext.currentTime;
...
Synth.prototype.noteOff = function(note, velocity){
...
Synth.prototype.setFilterFrequency = function(value){
  ...
  var now = window.audioContext.currentTime;
  var minValue = 40;
  var maxValue = window.audioContext.sampleRate / 2;
